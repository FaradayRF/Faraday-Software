...
class proxyio(object):
    ...
    def GET(self, uart_service_number):
        """
    ...
    def GETWait(self, uart_service_number, sec_timeout, debug):
        """
        ...
        while((rx_data == False) and (timedelta<sec_timeout)):
            ...
            #Attempt to get data
            rx_data = self.GetPortJson(uart_service_number)
    ...
    def FlushRxPort(self, uart_service_number):
        """
    ...
    def ParseTelemetryPacket3(self, data):
        """
        This function returns the python Struct results of parsion Telemetry Packet #3 in a supplied Telemetry Packet #3 raw packet.
        """
        telem_struct = struct.Struct('>9s 2B 9s 8B 1H 9s 1B 10s 1B 8s 1B 5s 1c 4s 3B 18s 2B 2H 24x 2B') #Struct definition of telemetry packet
        parsed_data_packet = telem_struct.unpack(data)
        return parsed_data_packet
