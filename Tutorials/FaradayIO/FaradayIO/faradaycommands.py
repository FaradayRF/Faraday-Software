...
class FaradayCommands(object):
    ...
    def CommandLocal(self, command_number, command_packet):
        """
        A predefined function that returns a raw command application DATAGRAM that will cause the supplied command to
        be executed on a local device.

        :param command_number: Command application command identification number (Identifies a command/command
            type using 0-255)
    ...
    def CommandRf(self, remote_callsign, remote_node_id, command_number, command_packet):
        """
        A predefined function that returns a raw command application DATAGRAM that will cause the supplied command to
        be executed on a remote (RF) device. This is basically and local command inside the payload of a command to
        transmit a command to a specific RF unit.
        ...
        :param command_number: Command application command identification number (0-255)
        :param command_packet: Pass a pre-generated packet as defined by the intended command.
    ...
    def CommandLocalUpdateRFFreq(self, freq_mhz):
        """
        A predefined command to return a command complete DATAGRAM and packet that will command a change in the
        LOCAL faraday radio frequency.

        :param freq_mhz: The frequency in MHz as an integer or float.

        :Return: Returns the complete generated packet as a string of bytes.
        """
        packet = commandmodule.create_command_datagram(self.CMD_UPDATEFREQUENCY,
                                                       commandmodule.create_update_rf_frequency_packet(freq_mhz))
        return packet

    def CommandLocalUARTUpdateNow(self):
        """
        A predefined command to return a complete command datagram and packet to command the LOCAL device to transmit
        its current telemetry packet #3 (normal telemetry) over UART.

        :Return: Returns the complete generated packet as a string of bytes.
        """
        packet = commandmodule.create_command_datagram(self.CMD_LOCALTELEMNOW,
                                                       commandmodule.create_local_telem_update_packet())
        return packet

    def CommandLocalRFUpdateNow(self):
        """
        A predefined command to return a complete command datagram and command packet to command the LOCAL device
        to transmit is current telemetry packet #3 (normal telemetry) over RF.

        :Return: Returns the complete generated packet as a string of bytes.
        """
        packet = commandmodule.create_command_datagram(self.CMD_RFTELEMETRYNOW,
    ...
    def CommandLocalGPIOLED1On(self):
        """
        A predefined command to return a complete datagram and command packet to turn ON the LOCAL Faraday's LED #1
        using the standard Faraday GPIO commanding packet.

        :Return: Returns the complete generated packet as a string of bytes.

        """
        packet = commandmodule.create_command_datagram(self.CMD_GPIO, commandmodule.create_gpio_command_packet(
            int(gpioallocations.LED_1), 0, 0, 0, 0, 0))
        return packet

    def CommandLocalGPIOLED1Off(self):
        """
        A predefined command to return a complete datagram and command packet to turn OFF the LOCAL Faraday's LED #1
        using the standard Faraday GPIO commanding packet.
    ...
    def CommandLocalGPIOLED2On(self):
        """
        A predefined command to return a complete datagram and command packet to turn ON the LOCAL Faraday's LED #2
        using the standard Faraday GPIO commanding packet.
        ...
        """
        packet = commandmodule.create_command_datagram(self.CMD_GPIO, commandmodule.create_gpio_command_packet(
            int(gpioallocations.LED_2), 0, 0, 0, 0, 0))
        return packet

    def CommandLocalGPIOLED2Off(self):
        """
        A predefined command to return a complete datagram and command packet to turn OFF the LOCAL Faraday's LED #2
        using the standard Faraday GPIO commanding packet.
    ...
    def CommandLocalResetDeviceDebugFlash(self):
        """
        A predefined command to return a complete datagram and command packet that commands the LOCAL Faraday device
        to RESET it's "Flash Debug Information" to 0's.

        Flash debug information is engineering data that contains information about the devices boot, reset, and
        error statuses. This information is saved in non-volatile flash and will roll over to 0 if 255 is reached.

        :Return: Returns the complete generated packet as a string of bytes.

        """
        packet = commandmodule.create_command_datagram(self.CMD_DEBUGFLASHRESET,
                                                       commandmodule.create_reset_device_debug_flash_packet())
        return packet

    def CommandLocalSendTelemDeviceDebugFlash(self):
        """
        A predefined command to return a complete datagram and command packet that causes LOCAL Faraday device to
        transmit it's "Flash Debug Information" over UART.
        ...
        """
        packet = commandmodule.create_command_datagram(self.CMD_DEBUGFLASHTELEMETRYNOW,
                                                       commandmodule.create_send_telemetry_device_debug_flash())
        return packet

    def CommandLocalSendTelemDeviceSystemSettings(self):
        """
        A predefined command to return a complete datagram and command packet to command the LOCAL Faraday device
        to transmit it's "Device System Settings" over UART.
        ...
        """
        packet = commandmodule.create_command_datagram(self.CMD_DEVICESETTINGSTELEMETRYNOW,
                                                       commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandLocalHABActivateCutdownEvent(self):
        ...
        packet = commandmodule.create_command_datagram(self.CMD_APP_HAB_CUTDOWNNOW,
                                                       commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandLocalHABResetAutoCutdownTimer(self):
        ...
        """
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_command_datagram(self.CMD_APP_HAB_RESETAUTOCUTDOWNTIMER,
                                                       commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandLocalHABDisableAutoCutdownTimer(self):
        ...
        """
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_command_datagram(self.CMD_APP_HAB_DISABLEAUTOCUTDOWNTIMER,
                                                       commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandLocalHABResetCutdownIdle(self):
        ...
        """
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_command_datagram(self.CMD_APP_HAB_RESETCUTDOWNSTATEMACHINE,
                                                       commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandLocalExperimentalRfPacketForward(self, remote_callsign, remote_node_id, data):
        """
        A predefined command to return a complete command datagram and command packet that commands a LOCAL Faraday
        device to send the provided packet over a single RF packet.
        ...
        """
        packet = commandmodule.create_command_datagram(self.CMD_RFPACKETFORWARD,
                                                       commandmodule.CreatePacketMsgExperimental(0, remote_callsign,
                                                                                                 remote_node_id,
                                                                                                 len(data), data))
        return packet

    def CommandLocalGPIO(self, p3_bitmask_on, p4_bitmask_on, p5_bitmask_on, p3_bitmask_off, p4_bitmask_off,
                         p5_bitmask_off):
        """
        A predefined command to return a complete command datagram and command packet to create a generic
        GPIO ON/OFF bitmask command.

        :param p3_bitmask_on: A 1 byte bitmask for PORT 3 GPIO that if bit HIGH it will toggle the
            corresponding GPIO HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p4_bitmask_on: A 1 byte bitmask for PORT 4 GPIO that if bit HIGH it will toggle the
            corresponding GPIO HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p5_bitmask_on: A 1 byte bitmask for PORT 5 GPIO that if bit HIGH it will toggle the
            corresponding GPIO HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p3_bitmask_off: A 1 byte bitmask for PORT 3 GPIO that if bit HIGH it will toggle the
            corresponding GPIO LOW. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p4_bitmask_off: A 1 byte bitmask for PORT 4 GPIO that if bit HIGH it will toggle the
            corresponding GPIO LOW. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p5_bitmask_off: A 1 byte bitmask for PORT 5 GPIO that if bit HIGH it will toggle the
            corresponding GPIO LOW. If bit is LOW it will have no affect on the corresponding GPIO.
        ...
        """
        packet = commandmodule.create_command_datagram(self.CMD_GPIO,
                                                       commandmodule.create_gpio_command_packet(p3_bitmask_on,
                                                                                                p4_bitmask_on,
                                                                                                p5_bitmask_on,
                                                                                                p3_bitmask_off,
                                                                                                p4_bitmask_off,
    ...
    def CommandLocalSendReadDeviceConfig(self):
        """
        A function returns a complete command datagram and command packet to read the device flash configuration from
        the local device. This function implements the memory read Faraday Python routine to read memory addresses with
        predefined arguments.

        :Return: Returns the complete generated packet as a string of bytes.

        """
        packet = commandmodule.create_command_datagram(self.CMD_READMEMORY,
                                                       commandmodule.create_read_memory_packet(self.FLASHD_MEMORY_LOC,
                                                                                               self.FLASHD_LENGTH))
        return packet
    ...
    def CommandRemoteUARTUpdateNow(self, remote_callsign, remote_node_id):
        """
        A predefined command to return a complete datagram and command packet to command the REMOTE (RF) device to
        transmit its current telemetry packet #3 (normal telemetry) over UART.
        ...
        """
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id, self.CMD_LOCALTELEMNOW,
                                                          commandmodule.create_local_telem_update_packet())
        return packet
    ...
    def CommandRemoteRFUpdateNow(self, remote_callsign, remote_node_id):
        """
        A predefined command to return a complete datagram and command packet to command the REMOTE (RF) device to
        transmit is current telemetry packet #3 (normal telemetry) over RF.
        ...
        """
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id, self.CMD_RFTELEMETRYNOW,
                                                          commandmodule.create_rf_telem_update_packet())
        return packet

    def CommandRemoteGPIOLED1On(self, remote_callsign, remote_node_id):
        """
        A predefined command to return a complete datagram and command packet to turn ON the REMOTE (RF) Faraday's
        LED #1 using the standard Faraday GPIO commanding packet.
        ...
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id, self.CMD_GPIO,
                                                          commandmodule.create_gpio_command_packet(
                                                              int(gpioallocations.LED_1), 0, 0, 0, 0, 0))
        return packet

    def CommandRemoteGPIOLED1Off(self, remote_callsign, remote_node_id):
        """
        A predefined command to return a complete datagram and command packet to turn OFF the REMOTE (RF) Faraday's
        LED #1 using the standard Faraday GPIO commanding packet.
        ...
        """
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id, self.CMD_GPIO,
                                                          commandmodule.create_gpio_command_packet(0, 0, 0, int(
                                                              gpioallocations.LED_1), 0, 0))
        return packet

    def CommandRemoteGPIOLED2On(self, remote_callsign, remote_node_id):
        """
        A predefined command to return a complete datagram and command packet to turn ON the REMOTE (RF) Faraday's
        LED #2 using the standard Faraday GPIO commanding packet.
    ...
    def CommandRemoteHABActivateCutdownEvent(self, remote_callsign, remote_node_id):
        ...
        """
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id, self.CMD_APP_HAB_CUTDOWNNOW,
                                                          commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandRemoteHABResetAutoCutdownTimer(self, remote_callsign, remote_node_id):
        ...
        """
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id,
                                                          self.CMD_APP_HAB_RESETAUTOCUTDOWNTIMER,
                                                          commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandRemoteHABDisableAutoCutdownTimer(self, remote_callsign, remote_node_id):
        ...
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id,
                                                          self.CMD_APP_HAB_DISABLEAUTOCUTDOWNTIMER,
                                                          commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandRemoteHABResetCutdownIdle(self, remote_callsign, remote_node_id):
        ...
        # Don't care what the payload is as long as it is at least 1 byte long
        packet = commandmodule.create_rf_command_datagram(remote_callsign, remote_node_id,
                                                          self.CMD_APP_HAB_RESETCUTDOWNSTATEMACHINE,
                                                          commandmodule.create_send_telemetry_device_debug_flash())
        return packet
    ...
    def CommandRemoteGPIO(self, remote_callsign, remote_node_id, p3_bitmask_on, p4_bitmask_on, p5_bitmask_on,
        ...
        :param remote_callsign: The callsign of the remote target Faraday device
        :param remote_node_id: The ID number (0-255) of the remote target Faraday device.
        :param p3_bitmask_on: A 1 byte bitmask for PORT 3 GPIO that if bit HIGH it will toggle the corresponding GPIO
            HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p4_bitmask_on: A 1 byte bitmask for PORT 4 GPIO that if bit HIGH it will toggle the corresponding GPIO
            HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p5_bitmask_on: A 1 byte bitmask for PORT 5 GPIO that if bit HIGH it will toggle the corresponding GPIO
            HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p3_bitmask_off: A 1 byte bitmask for PORT 3 GPIO that if bit HIGH it will toggle the corresponding GPIO
            LOW. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p4_bitmask_off: A 1 byte bitmask for PORT 4 GPIO that if bit HIGH it will toggle the corresponding GPIO
            LOW. If bit is LOW it will have no affect on the corresponding GPIO.
        :param p5_bitmask_off: A 1 byte bitmask for PORT 5 GPIO that if bit HIGH it will toggle the corresponding GPIO
            LOW. If bit is LOW it will have no affect on the corresponding GPIO.
