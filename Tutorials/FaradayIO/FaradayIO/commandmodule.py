...
def create_command_datagram(command, payload):
    """
    This function creates a command packet datagram that encapsulates the actual command packets to the parsed. This
    allows identification of packet "types" and modularity within the command system.

    :param command: The command "number" that identifies the command packet to be parsed or direct action to be taken
    :param payload: The payload for the specified command packet type. This is usually just commands full "packet"
...
def create_rf_command_datagram(dest_callsign, dest_device_id, command, payload):
    """
    This function creates a command packet datagram for commanding REMOTE (RF) Faraday devices. This datagram
    encapsulates the actual command packets to the passed to the remote device as a payload.

    The important difference between the RF and non RF command packet functions are remote callsign and ID arguments
    that tell Faraday what devices to target for commanding.

    .. note:: Remote commanding simply calls a specific local command that transmits a "local" command packet as a
        payload to the target Faraday device. Also, This command module has a predefined set of command number
        definitions for use/reference.
    ...
    if len(dest_callsign) < DEST_CALLSIGN_MAX_LEN:
        ...
        pkt_cmd_datagram_error_detection_struct = struct.Struct(
            '>1H')  # Command packect to be run on remote unit as local command Error Detection
        pkt_rf_cmd_struct = struct.Struct(
            '9s2B29s')  # RF Command packet that encapsulates local command to be run on remote unit
        ...
        pkt_cmd_datagram_error_detection = pkt_cmd_datagram_error_detection_struct.pack(
            checksum.compute_checksum_16(pkt_cmd_datagram, len(pkt_cmd_datagram)))
        ...
        pkt_rf_cmd = pkt_rf_cmd_struct.pack(str(dest_callsign).upper(), len(dest_callsign), dest_device_id,
                                            pkt_cmd_datagram_final)

        # Create final local command packet with Error Detection appended
...
def create_fixed_length_packet(data, fixed_length):
    """
    A simple function that accepts a string of data bytes and appends padding bytes up to a fixed length.

    :param data: The string of bytes that will be appended padding bytes to create the fixed length packet
    :param fixed_length: The length of the packet in total. Extra bytes not used will be padded.
    :return: Returns the "data" padded with additional bytes in the fixed sized specified.

    :Example:

        >>> create_fixed_length_packet("Test data message", 20)
        'Test data message\x00\x00\x00'

    .. note:: This should be updated so that the padding byte can be specified as well.
    """
    pad_len = fixed_length - len(data)
...
def create_freq_list(freq):
    ...
    :Return: A list of 3 bytes corresponding to the expected CC430 register settings for the given frequency. The fourth index item is the actual frequency set due to the PLL increment size.
...
def calc_radio_freq(freq0, freq1, freq2):
    """
    This function is a reverse frequency calculation routine that accepts the CC430 RF frequency register bytes and
    computes the effective frequency.
...
def create_gpio_cmd_bitmask(bit_number):
    """
    A simple function that creates a bitmask given the single bit location passed as an argument. This is useful
    for creating larger bitmasks specifically when creating bitmasks that control the GPIO functionality.
...
def create_gpio_command_packet(port3_on_bitmask, port4_on_bitmask, port5_on_bitmask, port3_off_bitmask,
                               port4_off_bitmask, port5_off_bitmask):
    """
    ...
    :param port3_on_bitmask: A 1 byte bitmask for PORT 3 GPIO that if bit HIGH it will toggle the corresponding GPIO HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
    :param port4_on_bitmask: A 1 byte bitmask for PORT 4 GPIO that if bit HIGH it will toggle the corresponding GPIO HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
    :param port5_on_bitmask: A 1 byte bitmask for PORT 5 GPIO that if bit HIGH it will toggle the corresponding GPIO HIGH. If bit is LOW it will have no affect on the corresponding GPIO.
    :param port3_off_bitmask: A 1 byte bitmask for PORT 3 GPIO that if bit HIGH it will toggle the corresponding GPIO LOW. If bit is LOW it will have no affect on the corresponding GPIO.
    :param port4_off_bitmask: A 1 byte bitmask for PORT 4 GPIO that if bit HIGH it will toggle the corresponding GPIO LOW. If bit is LOW it will have no affect on the corresponding GPIO.
    :param port5_off_bitmask: A 1 byte bitmask for PORT 5 GPIO that if bit HIGH it will toggle the corresponding GPIO LOW. If bit is LOW it will have no affect on the corresponding GPIO.
    ...
    if (check_on_int & check_off_int) != 0:
        ...
        gpio_cmd_pkt = gpio_cmd_pkt_struct.pack(port3_on_bitmask, port4_on_bitmask, port5_on_bitmask, port3_off_bitmask,
                                                port4_off_bitmask, port5_off_bitmask)
...
def create_read_memory_packet(dec_address, length):
    ...
    .. note:: The length cannot be larger than the maximum transmissible unit.

    :param dec_address: the starting location memory address (integer) to be returned
...
def create_update_rf_patable_packet(uchar_pa_table_setting):
    """
    ...
    :param uchar_pa_table_setting: The RF power table register setting byte for the CC430.

    :Return: A completed packet (string of bytes)

    """
    packet_struct = struct.Struct('1B')
    packet = packet_struct.pack(uchar_pa_table_setting)
    return packet
...
## Note: Not payload packet needed, returning 1 byte of 0 just for placeholder
##############
...
def CreatePacketMsgExperimental(msg_cmd, dest_callsign, dest_device_id, data_len, data):
    """
    This function creates the command packet expected by the command application routine that forwards a supplied
    payload over RF to a remote Faraday device. This is a very useful yet simple and un-optimized method of sending
    arbitrary data from one unit to another.

    :param msg_cmd: Byte indicating the command to be performed specific to the experimental RF packet forward function.
    :param dest_callsign: The callsign of the target Faraday unit
    :param dest_device_id: The device ID number of the target Faraday unit
    ...
    .. note: The data payload length cannot be larger than the maximum transmissible unit. Also, data only needs to be
        a string of bytes so both strings and packed binary data are acceptable.
